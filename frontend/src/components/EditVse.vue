    ...
    <div class="col-12">
      <div class="col-12">
        ...
        <select class="m-1" style="height: 30px" v-model="state.selected_enterprise">
          <option value="" selected disabled>Выберите предприятие</option>
        ...
        <tr v-for="(vse_param, index) in state.active_vse_params" :key="'vse-param-'+index">
          <td>
            <select v-model="state.active_vse_params[index].productName">
              <option :key="'product-'+product.uuid" v-for="product in state.products" :value="product.productName">
                {{ product.productName }}
              </option>
            </select>
          </td>
...
function append_active_vse_param() {
  let newActiveVseParam = JSON.parse(JSON.stringify(emptyVseParam))
