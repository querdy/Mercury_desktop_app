...
from app.api_v1.schemas.research_schema import EnterpriseSchema, EnterpriseDeleteSchema
...
@router.post("/enterprise", status_code=status.HTTP_201_CREATED, response_model=EnterpriseSchema)
def create_enterprise_route(enterprise: EnterpriseSchema, db: Session = Depends(get_db)) -> Any:
    try:
...
def delete_enterprise_route(enterprise: EnterpriseDeleteSchema, db: Session = Depends(get_db)):
    if not delete_enterprise(enterprise_uuid=enterprise.uuid, db=db):
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY)


@router.get("/enterprise/{uuid}", status_code=status.HTTP_200_OK, response_model=EnterpriseSchema)
def get_enterprise_route(uuid: int, db: Session = Depends(get_db)):
    try:
...
@router.get("/enterprises", status_code=status.HTTP_200_OK, response_model=list[EnterpriseSchema])
async def get_enterprises_route(db: Session = Depends(get_db)) -> Any:
    enterprises = get_all_enterprise(db=db)
    return enterprises


@router.put("/enterprise", status_code=status.HTTP_200_OK, response_model=EnterpriseSchema)
async def update_enterprise_route(enterprise: EnterpriseSchema = Body(), db: Session = Depends(get_db)):
    try:
